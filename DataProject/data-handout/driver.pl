...
# Override the login name if the flag is set
if ($opt_l) {
    $login = $opt_l;
	check_login($login);
}
else {
    $login = getlogin() || (getpwuid($<))[0] || "unknown";
}

$tmpdir = "/var/tmp/datalab.$login.$$";
...
#
# check_login - Check login for legality
#
sub check_login {
    my $login = shift;
    ...
    if (length($login) < 1) {
        die "$0: Error: Empty login.\n";
    }

    # login can't be too long
    if (length($login) > 35) {
        die "$0: Error: login exceeds 35 characters.\n";
    }

    # login can have restricted set of metacharacters (e.g., no #
    # HTML tags)
    if (!($login =~ /^[_-\w.,'@]+$/)) {
        die "$0: Error: Illegal character in nickname. Only alphanumerics, apostrophes, commas, periods, dashes, underscores, and ampersands are allowed.\n";
    }
